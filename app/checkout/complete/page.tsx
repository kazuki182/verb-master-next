...
export default function CheckoutCompletePage() {
  ...
  useEffect(() => {
    ...
    const progress = setUserUnlockLevel(
      ...
      plan,
      "checkout_ready",
      `仮購入：${planLabels[plan]}を反映しました。正式決済接続前の確認用です。`,
    );
  ...
  const label = useMemo(() => planLabels[targetPlan] || `${targetPlan}動詞パック`, [targetPlan]);
    ...
    <div className="space-y-5 pb-24">
      ...
      <section className="card p-5">
        ...
        <div className="mt-4 grid gap-3 text-sm">
          <div className="rounded-2xl bg-paper p-4 font-bold">プラン：{label}</div>
